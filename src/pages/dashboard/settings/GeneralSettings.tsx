...
import EditAccountModal from '../client/account/EditAccountModal'; // Reutilizar modal
import ChangePasswordModal from '../client/account/ChangePasswordModal'; // Reutilizar modal
import { fetchUserProfile } from '../../../api/userApi'; // Para obtener el perfil del administrador
import {
...
// Hook para detectar el tema del dashboard (copiado de ClientAccount)
...
const GeneralSettings: React.FC = () => {
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        ...
        console.log('Admin Data from localStorage:', adminData); // DEBUG

        if (!adminData) {
            console.error('No hay datos de administrador en localStorage.');
            navigate('/');
            return;
        }

        const parsedAdmin = JSON.parse(adminData);
        console.log('Parsed Admin Data:', parsedAdmin); // DEBUG

        if (!parsedAdmin || !parsedAdmin.id) {
            console.error('Datos de administrador incompletos en localStorage.');
            navigate('/');
            return;
        }

        const fullProfile = await fetchUserProfile(parsedAdmin.id);
        console.log('GeneralSettings: Full user profile fetched:', fullProfile); // DEBUG
        
        localStorage.setItem('userData', JSON.stringify(fullProfile));
        setUser(fullProfile);
      } catch (error) {
        console.error('GeneralSettings: Error al cargar perfil del administrador:', error);
        // Aquí podríamos mostrar un mensaje de error o redirigir
        navigate('/');
      } finally {
        setLoading(false);
      }
  ...
  // Estilos según tema (copiado de ClientAccount)
  const getStyles = () => {
    if (theme === 'amanecer') {
      return {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <h2 className={`text-3xl font-black ${styles.title}`}>Ajustes de mi Cuenta</h2>
